...
$query2=$db->prepare("SELECT seasons.id AS temporada_id, episodes.title AS episodios FROM series LEFT JOIN seasons ON series.id=serie_id LEFT JOIN episodes ON season_id=seasons.id WHERE series.id=:id  ORDER BY seasons.id;"); // Me traigo los episodios
